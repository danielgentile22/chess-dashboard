...
Georgina Chin win with the documented 7 prose comments + 10 variations — plus
...
class TestParseCoachStudy:
    ...
    def test_chapter_carries_its_move_prefix(self):
        chapters = parse_coach_study(COACH_PGN)
        georgina = _by_name(chapters, "Georgina")
        # The Caro-Kann the real Game opened with (ADR 0001 move identity)
        assert georgina.move_prefix[:6] == ("e4", "c6", "Nc3", "d5", "d3", "dxe4")

    def test_chapter_preserves_annotated_movetext_for_the_board(self):
        georgina = _by_name(parse_coach_study(COACH_PGN), "Georgina")
        # The Coach board view replays the coach's annotated line, not a bare one
        assert "e4" in georgina.movetext
        assert georgina.movetext.strip()
...
class TestProseExtraction:
    def test_georgina_has_the_documented_seven_comments(self):
        georgina = _by_name(parse_coach_study(COACH_PGN), "Georgina")
        assert len(georgina.comments) == 7

    def test_engine_eval_directives_are_stripped_but_prose_kept(self):
        georgina = _by_name(parse_coach_study(COACH_PGN), "Georgina")
        texts = [c.text for c in georgina.comments]
...
class TestVariations:
    def test_georgina_preserves_its_ten_variations(self):
        georgina = _by_name(parse_coach_study(COACH_PGN), "Georgina")
        assert georgina.variation_count == 10
