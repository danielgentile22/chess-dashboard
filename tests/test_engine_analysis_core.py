...
Georgina Chin Game) and small synthetic analysed movetext for the attribution
...
GEORGINA_PGN = (FIXTURES / "analyzed-georgina-chin.pgn").read_text()
GEORGINA_URL = "https://lichess.org/study/6jYtXHGp/GKSICQAY"


def _georgina() -> GameAnalysis:
    """The analysed Georgina Chin Game, parsed the way a Sync produces it."""
...
class TestWinPctFormula:
    ...
    @pytest.mark.parametrize("cp, expected", [
        ...
        (-100, 40.90),
        (-300, 24.89),
        (54, 54.95),    # the position before Georgina's 16. Bd4
...
# ---------------------------------------------------------------------------
# The whole parser against the real analysed Georgina Chin Game
# ---------------------------------------------------------------------------
...
class TestErrorProfile:
    ...
    On the analysed Georgina Chin Game, Daniel is Black and *won* (0-1); his one
...
class TestSyncIntegration:
    ...
    def test_mistake_type_distribution_comes_from_the_store(self, store_from_pgn):
        data = store_from_pgn(GEORGINA_PGN)
        # Daniel's only mistake in the analysed Georgina Game is the positional g5.
        assert data.get_mistake_type_distribution() == {
