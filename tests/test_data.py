...
class TestInitialize:
    def test_boots_from_a_lichess_study(self, sample_pgn_text):
        ...
        with mock.patch.object(
            ...
            df, player = data.initialize("abcdWXYZ", player_name="Test Player")

        fetch.assert_called_once()
        assert fetch.call_args.args[0] == "abcdWXYZ"
        assert len(df) == 7
    ...
    def test_api_token_passed_to_client(self, sample_pgn_text):
        with mock.patch.object(
            ...
            data.initialize("abcdWXYZ", player_name="Test Player", token="lip_tok")
