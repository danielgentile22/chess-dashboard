...
test: install-dev  ## Run the test suite
	$(PYTEST) tests/ -v --cov=pgn_stats_core --cov=lichess_client --cov=data --cov-report=term-missing

lint: install-dev  ## Lint with ruff
	$(RUFF) check . --fix

typecheck: install-dev  ## Type check with mypy
	$(VENV)/bin/mypy pgn_stats_core.py lichess_client.py data.py config.py
