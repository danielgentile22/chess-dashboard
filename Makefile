...
test: install-dev  ## Run the test suite
	$(PYTEST) tests/ -v --cov=pgn_stats_core --cov=lichess_client --cov=uscf_client --cov=uscf_core --cov=engine_analysis_core --cov=sync --cov=data --cov=config --cov=shell --cov=filters --cov=components --cov=pages --cov-report=term-missing
